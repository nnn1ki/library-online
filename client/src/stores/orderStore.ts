...
export const useOrderStore = defineStore("orderStore", () => {
    ...
    const allowedStatusesToCountOrderedBooks: OrderStatusEnum[] = ['new', 'processing', 'ready', 'done'];
    ...
    async function handleCreateOrder() {
        ...
        if (!isValid) {
            toast.error("У вас много заказанных книг и книг на руках, больше заказывать нельзя. Нужно вернуть часть книг в библиотеку.");
            return;
        }
    ...
    async function validateOrder(): Promise<boolean> {
        ...
        const totalItems = borrowedBooks.value.length - selectedBorrowedBooks.value.length + selectedBooks.value.length + orderedBooksCount;
